...
int main(void){
  ...
  while(1){          // Follows the nine steps list above
   ...
   WaitForASHigh();  // b) wait for switch to be pressed
   ClearReady();     // c) Ready signal goes low
   Delay1ms(10);     // d) wait 10ms
   WaitForASLow();   // e) wait for switch to be released
...
// write this function
    do{
     AS = GPIO_PORTF_DATA_R&0x10; // PF4 into SW1
    }while(!AS);		
}
...
void Delay1ms(unsigned long msec){unsigned long volatile time;
  while(msec>0){
